...
class PDFGenerator {
    ...
    generateHTML(data) {
        ...
        console.log(`📄 Génération HTML: ${orders.length} commandes, ${creditNotes.length} avoirs reçus`);
        ...
        const creditNotesHTML = creditNotes
            ...
            .join("");
        const totalOrdersAmount = orders.reduce((sum, order) => sum + parseFloat(String(order.totalAmountTTC || 0)), 0);
        const totalCreditNotesAmount = creditNotes.reduce((sum, creditNote) => sum + parseFloat(String(creditNote.totalAmountTTC || 0)), 0);
        console.log(`💰 Totaux calculés: ${totalOrdersAmount.toFixed(2)} € commandes, ${totalCreditNotesAmount.toFixed(2)} € avoirs`);
        return `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        ...
        <div class="footer">
          <p>Export généré le ${new Date().toLocaleDateString("fr-FR")} à ${new Date().toLocaleTimeString("fr-FR")}</p>
    ...
    generateOrderDetailsHTML(order) {
        ...
        const orderItemsHTML = order.items && order.items.length > 0
            ? order.items
                ...
                const totalVAT = parseFloat(item.totalPriceTTC || 0) -
        ...
        const addressesHTML = order.addresses && order.addresses.length > 0
            ? order.addresses
                .map((address) => {
                const addressType = address.addressType || address.type;
                return `
          <div class="address-block">
            <h4>${addressType === "billing"
      ...
      <div class="order-details">
        ...
        <div class="order-info">
          ...
          <table class="info-table">
            ...
            <tr><td><strong>Date de création:</strong></td><td>${order.createdAt instanceof Date
            ? order.createdAt.toLocaleDateString("fr-FR")
            : new Date(String(order.createdAt)).toLocaleDateString("fr-FR")}</td></tr>
            <tr><td><strong>Méthode de paiement:</strong></td><td>${String(order.paymentMethod || "N/A")}</td></tr>
            <tr><td><strong>Statut:</strong></td><td>${Boolean(order.delivered) ? "Livrée" : "En attente"}</td></tr>
            <tr><td><strong>Total HT:</strong></td><td>${Number(order.totalAmountHT || 0).toFixed(2)} €</td></tr>
            <tr><td><strong>Total TTC:</strong></td><td>${Number(order.totalAmountTTC || 0).toFixed(2)} €</td></tr>
            ${order.notes
    ...
    generateCreditNoteDetailsHTML(creditNote) {
        ...
        const creditNoteItemsHTML = creditNote.items && creditNote.items.length > 0
            ? creditNote.items
                .map((item) => {
                const vatRate = parseFloat(item.vatRate || 0);
                const totalVAT = parseFloat(item.totalPriceTTC || 0) -
      ...
      <div class="credit-note-details">
        ...
        <div class="credit-note-info">
          <h3>Informations générales</h3>
          <table class="info-table">
            <tr><td><strong>ID Avoir:</strong></td><td>${creditNote.id}</td></tr>
            <tr><td><strong>Commande associée:</strong></td><td>${creditNote.orderId || "N/A"}</td></tr>
            <tr><td><strong>Raison:</strong></td><td>${creditNote.reason || "N/A"}</td></tr>
            <tr><td><strong>Description:</strong></td><td>${creditNote.description || "N/A"}</td></tr>
            <tr><td><strong>Méthode de paiement:</strong></td><td>${String(creditNote.paymentMethod || "N/A")}</td></tr>
            <tr><td><strong>Date de création:</strong></td><td>${creditNote.createdAt instanceof Date
            ? creditNote.createdAt.toLocaleDateString("fr-FR")
            : new Date(String(creditNote.createdAt)).toLocaleDateString("fr-FR")}</td></tr>
            <tr><td><strong>Total HT:</strong></td><td>${Number(creditNote.totalAmountHT || 0).toFixed(2)} €</td></tr>
            <tr><td><strong>Total TTC:</strong></td><td>${Number(creditNote.totalAmountTTC || 0).toFixed(2)} €</td></tr>
            ${creditNote.notes
    ...
    generateInvoiceHTML(order) {
        ...
        `;
                ...
                .join("")
            : "<p>Aucune adresse</p>";
        const totalVAT = Number(order.totalAmountTTC || 0) - Number(order.totalAmountHT || 0);
        return `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        ...
        <div class="invoice-info">
          ...
          <div class="invoice-info-section">
            ...
            <p><strong>Date:</strong> ${order.createdAt instanceof Date
            ? order.createdAt.toLocaleDateString("fr-FR")
            : new Date(String(order.createdAt)).toLocaleDateString("fr-FR")}</p>
            <p><strong>Méthode de paiement:</strong> ${String(order.paymentMethod || "N/A")}</p>
            <p><strong>Statut:</strong> ${Boolean(order.delivered) ? "Livrée" : "En attente"}</p>
          </div>
        </div>
