...
export default class OrderService {
  ...
  async createOrder(data: OrderData): Promise<Order> {
    try {
      ...
      const result = await this.orderRepository.getByIdWithJoins(
        savedOrder.id!
      );
      if (!result) {
  ...
  async updateOrder(id: number, data: OrderUpdateData): Promise<Order> {
    try {
      ...
      const result = await this.orderRepository.getByIdWithJoins(id);
      if (!result) {
   ...
   */
  async addOrderItem(orderId: number, itemData: any): Promise<OrderItem> {
    try {
      // Verify order exists
   ...
   */
  async updateOrderItem(itemId: number, itemData: any): Promise<OrderItem> {
    try {
   ...
   */
  async recalculateOrderTotals(orderId: number): Promise<any> {
    try {
  ...
  async createCreditNote(data: CreditNoteData): Promise<CreditNote> {
    try {
      ...
      const result = await this.creditNoteRepository.getById(
        savedCreditNote.id!
      );
      if (!result) {
   ...
   */
  async updateCreditNote(id: number, data: any): Promise<CreditNote> {
    try {
      ...
      const result = await this.creditNoteRepository.getById(id);
      if (!result) {
   ...
   */
  async listCreditNotes(options: OrderListOptions = {}): Promise<any> {
    try {
   ...
   */
  async listCreditNotesByCustomer(customerId: number): Promise<CreditNote[]> {
    try {
   ...
   * @returns {Promise<CreditNote[]>} Array of credit notes
   */
  async listCreditNotesByOrder(orderId: number): Promise<CreditNote[]> {
    try {
   ...
   */
  async getCreditNoteStatistics(options: OrderListOptions = {}): Promise<any> {
    try {
   ...
   * @returns {Promise<boolean>} True if deleted successfully
   */
  async deleteCreditNoteItem(itemId: number): Promise<boolean> {
    try {
      const item = await this.creditNoteItemRepository.getById(itemId);
      if (!item) {
        throw new Error("Credit note item not found");
   ...
   */
  async listCreditNoteItems(creditNoteId: number): Promise<CreditNoteItem[]> {
    try {
   ...
   * @returns {Promise<Object>} Updated totals
   */
  async recalculateCreditNoteTotals(creditNoteId: number): Promise<any> {
    try {
