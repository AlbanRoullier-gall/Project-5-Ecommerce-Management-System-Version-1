...
import { AuthenticatedRequest, OrderListOptions } from "./types";
...
async function getCustomerDetails(customerId: number): Promise<any> {
  try {
    ...
    console.error("Error fetching customer details:", (error as Error).message);
    return null;
  }
}
...
app.get(
  ...
  async (req: any, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const order = await orderService.getOrderById(parseInt(id || "0"));
...
app.delete(
  ...
  async (req: any, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const deleted = await orderService.deleteOrder(parseInt(id || "0"));
