 ...
 *
 * PRINCIPES :
 * - Travaille uniquement avec les Models/Entities
 * - Ne contient pas de logique de conversion DTO
 * - Orchestre les repositories pour la logique métier
 * - Gère les tokens JWT et la sécurité
 */
import { Pool } from "pg";
...
export class AuthService {
  ...
  /**
   * Register a new user
   * @param {UserData} userData User data from database layer
  ...
  async registerUser(
    ...
    try {
      ...
      // Validate input using model methods
      ...
      // Save user via repository (returns Model)
   ...
   * Update user
   * @param {number} id User ID
   * @param {Partial<UserData>} updateData User data to update
   * @returns {Promise<User>} Updated user
   */
  async updateUser(id: number, updateData: Partial<UserData>): Promise<User> {
    try {
      const user = await this.userRepository.getById(id);
      if (!user) {
        throw new Error("User not found");
      ...
      const updatedUser = new User({
        ...
        password_hash: user.passwordHash, // Keep existing password
        first_name: updateData.first_name || user.firstName,
        last_name: updateData.last_name || user.lastName,
      ...
      // Save changes via repository (returns Model)
  ...
  async changePassword(
    ...
    try {
      ...
      // Hash new password using model method
      const newPasswordHash = await User.hashPassword(newPassword);

      // Create updated user model with new password
      const updatedUser = new User({
        user_id: user.userId,
        email: user.email,
  ...
  async resetPassword(token: string, newPassword: string): Promise<boolean> {
    try {
      ...
      // Get user model to create updated user
      const user = await this.userRepository.getById(reset.userId);
      if (!user) {
        throw new Error("User not found");
      }
